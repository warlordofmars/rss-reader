...
def test_delete_feed_not_found(client, auth_headers):
    resp = client.delete("/feeds/9999", headers=auth_headers)
    assert resp.status_code == 404


def test_delete_feed_wrong_user(client, db, feed):
    from db import User
    from main import create_jwt

    other = User(google_id="other-456", email="other@example.com", name="Other", picture="")
    db.add(other)
    db.commit()
    db.refresh(other)
